    ...
    while(!isDone){
        ...
        for(int ones = 0; ones < (int)currTable.size() -1; ++ones){
            for(auto& imp: currTable[ones]){
                ...
                if(ENABLECOUNTPRIME && imp.isMerge){continue;}
...
void QM::genCoverSet(){
    ...
    isCover = std::vector<bool>((int)pow(2, nBit) + 1, false);
    for(auto& imp: onSet){unCovered.push_back(imp.bits);} 
    for(int i = 0; i < (int)primeSet.size(); ++i){
        std::vector<uint32_t> coverNum;
        // for(auto& n: primeSet[i].containNum){
        //     coverNum.push_back(n);
        //     reverseCoverTable[n].push_back(i);
        //     ++ElementToImp[n];
    ...
    int count = 0;
    int remainCount = onSet.size();
    while(remainCount > 0){
        ++count;
        // if(count % 10000 ==0){std::cout<<remainCount<<std::endl;}
        ...
        countTable[impID] = 0;
        for(auto& n: coverTable[impID]){
            // std::cout<<n<<" ";
            if(isCover[n]){continue;}
            --remainCount;
            removeNum(n);
        }
        // std::cout<<std::endl;
...
void multiQM::readFile(std::string inputFile){
    ...
    // std::cout<<onSet.size()<<" "<<dcSet.size()<<std::endl;
}
...
void multiQM::dumpCover(std::string outputFile){
    ...
    histPrime.reserve(coverSet.size());
